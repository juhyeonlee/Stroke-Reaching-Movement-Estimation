...
resample_num = 300
if not os.path.exists('whole_ts_{}_{}to{}_norm{}_avg'.format(f_lp, f_bp_l, f_bp_h, resample_num)):
    os.makedirs('whole_ts_{}_{}to{}_norm{}_avg'.format(f_lp, f_bp_l, f_bp_h, resample_num))
...
fig_vel_avg_normmag = plt.figure(figsize=[15, 10])
...
for comp in range(4):
    ...
    # for ii in range(100):
          reach_mvel_xy_df = reach_mvel_xy_df.append(
            pd.DataFrame({"t": np.arange(len(mvel_xy_reach_norm[comp][ii])), "vel": mvel_xy_reach_norm[comp][ii], "index": ii}))

    for ii in range(len(mvel_xy_retract_comp[comp])):
    # for ii in range(64):
        retract_mvel_xy_df = retract_mvel_xy_df.append(
            pd.DataFrame(
                {"t": np.arange(len(mvel_xy_retract_norm[comp][ii])), "vel": mvel_xy_retract_norm[comp][ii], "index": ii}))

    reach_mvel_xy_ax = fig_vel_avg_normmag.add_subplot(2, 4, comp + 1)
    ...
    retract_mvel_xy_ax = fig_vel_avg_normmag.add_subplot(2, 4, comp + 5)
...
# plt.show()
fig_vel_avg_normmag.savefig('whole_ts_{}_{}to{}_norm{}_avg.png'.format(f_lp, f_bp_l, f_bp_h, resample_num ))
plt.close('all')
