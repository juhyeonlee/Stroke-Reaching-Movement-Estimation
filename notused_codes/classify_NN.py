...
for s in subject_id:
    ...
    #
    #         val_model = GaussianProcessClassifier(kernel=kernel, random_state=0, n_restarts_optimizer=10, max_iter_predict=100)
    ...
    #     j += 1
    # best_n_keep = selnum_f_grid[int(np.argmax(val_scores))]
    # print(np.max(val_scores), best_n_keep)
    #
    # fsel = SelectKBest(f_classif, k=10).fit(train_feats, train_labels)
    ...
    model = Net(train_feats.shape[1], 2)
    train_NN(model, train_feats, train_labels)
    y_pred_train, _ = test_NN(model, train_feats)
    y_pred, y_pred_score = test_NN(model, test_feats)
    predicted[all_sub == s] = y_pred
    predicted_testscore[all_sub == s] = y_pred_score[:, 1]
    print('train acc:', accuracy_score(train_labels, y_pred_train))
