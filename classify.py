...
p_grid ={'C': [0.1, 1, 10], 'gamma': [0.0001, 0.001, 0.01]}
r_grid = [int(x) for x in np.linspace(start=200, stop=2000, num=10)]
selnum_f_grid = np.arange(1, len(feature_names))
...
for s in subject_id:
        ...
        for cval in p_grid['C']:
            for gamval in p_grid['gamma']:
                ...
                for i_s in inner_subject_id:
                    ...
                    # CFS
                    # fs = CFS().fit(inner_train_feats, inner_train_labels, n_keep)
                    # inner_train_feats = fs.transform(inner_train_feats)
                    # f_name = fs.get_features_name(feature_names)
                    # print(f_name)
                    ...
                    val_model = SVC(C=cval, gamma=gamval, kernel='rbf', random_state=0, class_weight='balanced')
                    val_model.fit(inner_train_feats, inner_train_labels)
                    predicted_val[train_sub == i_s] = val_model.predict(val_feats)
                val_scores[j] = f1_score(train_labels, predicted_val)
                j += 1
    ...
    print(feature_names[fsel.top_features])
    test_feats = fsel.transform(test_feats)

    # cfs_idx = CFS.cfs(train_feats, train_labels)
    # fs = CFS().fit(train_feats, train_labels, best_n_keep)
    ...
    model = SVC(kernel='rbf', random_state=0, class_weight='balanced', C=best_C, gamma=best_gamma)
...
fig_conf.savefig('confusion_matrix_2class')
np.save('fcbf_F1', predicted)
